...
void setup() {
  ...
  analogWrite(pinSpeed_Left,50); // set speed reference, duty-cycle = 100/255
  analogWrite(pinSpeed_Right,48); 
  interrupts(); 
}
...
void TurnRight(int degrees){
      float rots = float(degrees)/360; //only do FP division once. convert degrees to rotations
      int count = int(768.0*rots); //64 encoder pulses per rotation
      lcount = 0; 
      ...
      digitalWrite(pinCW_Right,LOW);  // start turning right 
...
void GoForward(int degrees){
      ...
      digitalWrite(pinCW_Right,HIGH); 
      digitalWrite(pinCC_Right, LOW);  
      while(lcount<=count); //We're spinning here which is power intensive but hopefully ok. (Left count could be used as well) 
      Serial.println(lcount); 
      digitalWrite(pinCC_Left,LOW);
      digitalWrite(pinCW_Left,LOW); 
      digitalWrite(pinCW_Right,LOW);  // start turning right 
...
void loop() {
  ...
  delay(1000); 
  GoForward(500); //need conversion from degrees to feet 
  TurnRight(760); 
  GoForward(500); 
  TurnLeft(740); 
}
